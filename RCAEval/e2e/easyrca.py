...
def easyrca(
    ...
    def run_pcmci(data, pc_alpha=0.1, verbosity=0):
        ...
        pcmci_res = pcmci.run_pcmciplus(tau_max=10, pc_alpha=pc_alpha)
    ...
    for name_y in pcmci.all_parents.keys():
        for name_x, t_xy in pcmci.all_parents[name_y]:
            if (node_names[name_x], node_names[name_y]) not in g.edges:
                g.add_edges_from([(node_names[name_x], node_names[name_y])])


    anomalous = node_names
    anomalies_start = dict()
    for node in anomalous:
        anomalies_start[node] = inject_time

    anomaly_size = 400

    AG = EasyRCA(g, anomalous, anomalies_start_time=anomalies_start, anomaly_length=anomaly_size)
    AG.run_without_data()
